...
inline void ucs4_from_ucs1(void* z, const void* q, int32 n) { copy_and_convert(u32ptr(z), cu8ptr(q), n); }

inline void ucs4_from_ucs2(void* z, const void* q, int32 n) { copy_and_convert(u32ptr(z), cu16ptr(q), n); }

inline void ucs4_from_ucs4(void* z, const void* q, int32 n) { copy_and_convert(u32ptr(z), cu32ptr(q), n); }

inline void ucs2_from_ucs1(void* z, const void* q, int32 n) { copy_and_convert(u16ptr(z), cu8ptr(q), n); }

inline void ucs2_from_ucs2(void* z, const void* q, int32 n) { copy_and_convert(u16ptr(z), cu16ptr(q), n); }

inline void ucs2_from_ucs4(void* z, const void* q, int32 n) { copy_and_convert(u16ptr(z), cu32ptr(q), n); }

inline void ucs1_from_ucs1(void* z, const void* q, int32 n) { copy_and_convert(u8ptr(z), cu8ptr(q), n); }

inline void ucs1_from_ucs2(void* z, const void* q, int32 n) { copy_and_convert(u8ptr(z), cu16ptr(q), n); }

inline void ucs1_from_ucs4(void* z, const void* q, int32 n) { copy_and_convert(u8ptr(z), cu32ptr(q), n); }
...
static CharSize utf8_req_charsize(cptr a, cptr e)
{
	...
	while (a < e && uchar(*a) < 0xC4) { a++; }
	if (a == e) return csz1;
	while (a < e && uchar(*a) < 0xF0) { a++; }
	if (a == e) return csz2;
	else return csz4;
}

static inline CharSize req_charsize(ucs4char n) { return n >> 16 ? csz4 : n >> 8 ? csz2 : csz1; }

__attribute__((__unused__)) static inline CharSize req_charsize(ucs2char n) { return n >> 8 ? csz2 : csz1; }
...
static ucs1char* ucs1_from_utf8(ucs1char* z, cstr a, cstr e, bool latin_1 = no)
{
	...
	while (a < e)
	{
		if (utf8::is_7bit(c1 = *a++))
		{
			*z++ = ucs1char(c1);
			continue;
		}
		if (utf8::is_fup(c1)) { continue; }
		if (a >= e || utf8::no_fup(c2 = *a) || uchar(c1) >= 0xc4)
		{
			*z++ = latin_1 ? ucs1char(c1) : '?';
			continue;
		}
		a++;
		*z++ = ucs1char((c1 << 6) + (c2 & 0x3f));
	}
	return z;
}
...
static ucs2char* ucs2_from_utf8(ucs2char* z, cstr a, cstr e, bool latin_1 = no)
{
	...
	while (a < e)
	{
		...
		if (uchar(c1) < 0xf0)
		{
			*z++ = ucs2char(((c1) << 12) + ((c2) << 6) + uchar(c3));
			continue;
		}
	x:
		*z++ = latin_1 ? ucs2char(c1) : '?';
	}
	return z;
}
...
static ptr utf8_from_ucs2(ptr z, const ucs2char* q, int32 n)
{
	...
	while (q < e)
	{
		...
		{
			*z++ = _utf8_starter_c3(c);
			*z++ = _utf8_fup(uint8(c >> 6));
			*z++ = _utf8_fup(uint8(c));
		}
	}
	return z;
...
static ptr utf8_from_ucs4(ptr z, const ucs4char* q, int32 n)
{
	...
	while (q < e)
	{
		...
		{	 // num fups = i =	15/6=2	20/6=3	25/6=4	30/6=5	35/6=5
			int i = 15;
			for (uint m = c >> 16; m; m >>= 5) { i += 5; }
			i	 = i / 6;									   // num bits in fups = i*6
...
void String::Check(cstr filename, uint line) const
{
	...
	if ((next != this) != (prev != this))
		abort("String::Check(): (next!=this)!=(prev!=this) in file %s line %u", filename, line);
...
bool FNMatch(cString& filename, cString& pattern, MatchType flags)
{
	...
	if (flags & fnm_pathname)
	{
		...
		while (p_rem.Len() > 1 && f_rem.Len() > 1)
		{
			...
			p	  = p_rem.LeftString(pi);
			p_rem = p_rem.MidString(pi + 1);
			f	  = f_rem.LeftString(fi);
			f_rem = f_rem.MidString(fi + 1);
...
*/
String::String() : text(nullptr), count(0), data_and_csz(calc_data_and_csz(nullptr, csz1)), next(this), prev(this)
{
	xlogIn("String::String()");
}
...
void String::Truncate(int32 newlen)
{
	...
	else if (newlen < count) { count = newlen; }
}
...
void String::Resize(int32 newlen, ucs4char padding)
{
	...
	if (newlen <= count) { Truncate(newlen); }
	else
	{
		XXXCHECK(*this);
		CharSize ocsz	 = Csz();
...
double String::NumVal(int32* idx_io) const
{
	...
	{
		int				i;
		const ucs4char* p4 = ucs4_text + idx;
		for (i = 0; i < n; i++)
		{
			if (p4[i] >> 8) break;
		}
		q = bu;
		n = i;
		ucs1_from_ucs4(bu, p4, n);
		break;
	}
	case csz2:
	{
		int				i;
		const ucs2char* p2 = ucs2_text + idx;
		for (i = 0; i < n; i++)
		{
			if (p2[i] >> 8) break;
		}
	...
	*uptr(qe) = 0;
	...
	if (is_dec_digit(c)) // 123.456e78
	{
		...
		else
		{
			...
			if (errno && (errno == HUGE_VAL || errno == ERANGE)) /*ForceError(numbertoobig);*/
				errno = ok;
		}
	...
	{
		a = q;
		while (q < qe && (n = _digit_value(*q)) < 16)
		{
			q++;
			f = ldexp(f, 4) + n;
		}
	...
	if (c == '\'') // 'abcd'
	{
		...
		for (;;)
		{
			...
			if (c == '\\')
			{
				esc = yes;
				q++;
			} // skip next byte. if multiple chars follow (e.g. octal digits) this will do fine too
			q++;
		}
		if (esc)
		{
...
String String::StrVal(int32* idx_io) const
{
	...
	ucs4char				 leftquote, rightquote;
	leftquote = rightquote = operator[](idx);
...
void String::Replace(cString& o, cString& n)
{
	...
	for (int32 i = 0; (i = Find(o, i)) >= 0; i += n.Len()) { *this = LeftString(i) + n + MidString(i + o.Len()); }
}
...
void String::Swap(ucs4char o, ucs4char n)
{
	...
	case csz1:
	{
		...
		{
			c = *e;
			if (c == ucs1char(o) || c == ucs1char(n)) *e = ucs1char(c ^ x);
		}
	}
		return;
	case csz2:
	{
		...
		{
			c = *e;
			if (c == ucs2char(o) || c == ucs2char(n)) *e = ucs2char(c ^ x);
		}
	}
		return;
	case csz4:
	{
		...
		{
			c = *e;
			if (c == ucs4char(o) || c == ucs4char(n)) *e = ucs4char(c ^ x);
		}
	}
		return;
	}
...
#define H(UTF8, BESCHR, NAME, UCS2) \
  {                                 \
	NAME, UCS2                      \
  }
	...
	case csz1:
	{
		...
		while (q < qe)
		{
			...
			if (*q == '#') // UCS4 number
			{
				...
				n = 0; // UCS4 code akku
				if ((*q | 0x20) == 'x')
					for (q++; (m = _digit_value(*q)) < 16; q++) { n = (n << 4) + m; }
				else
					for (; (m = _digit_val(*q)) < 10; q++) { n = n * 10 + m; }
				if (q != qs) goto x1;
			}
			else // named entity
			{
...
String String::FromEscaped(ucs4char leftquote) const
{
	...
	case csz1:
	{
		...
		if (leftquote && count >= 2 && *q == leftquote && qe[-1] == rightquote)
		{
			q++;
			qe--;
		}
...
String String::FromUTF8() const
{
	...
	if (len >= 3 && peek3X(text) == 0x00efbbbf)
	{
		text += 3;
		len -= 3;
	} // skip BOM
...
String String::FromUCS2() const
{
	...
	s.ResizeCsz(csz1);
	s.SetCsz(csz2);
	s.count /= csz2; // note: bogus final odd byte gets lost
	XXXCHECK(s);
	return s;
}
...
String String::FromUCS4() const
{
	...
	s.ResizeCsz(csz1);
	s.SetCsz(csz4);
	s.count /= csz4; // note: bogus final odd bytes get lost
	XXXCHECK(s);
	return s;
}
...
String String::FromUrl() const
{
	...
	for (int i = 0; i < z.Len() - 2; i++)
	{
		...
		if (c != '%') continue;
		char a = z.text[i + 1];
		if (no_hex_digit(a)) continue;
		char b = z.text[i + 2];
		if (no_hex_digit(b)) continue;
...
String String::ConvertedTo(CharEncoding e) const
{
	...
	b8:
	{
		...
		{
			cu8ptr q = Ucs1();
			for (int32 i = 0; i < Len(); i++) z[i] = ucs4::to_8bit(q[i], ctable);
		}
		break;
		case csz2:
		{
...
String HexString(double d, int digits) // ***TODO*** fractional part ?
{
	...
	do {
		s.Text()[--digits] = hex[uint(ldexp(modf(ldexp(d, -4), &d), 4))];
	}
	while (digits);
	return s;
}
...
String BinString(double d, int digits) // ***TODO*** fractional part ?
{
	...
	else
	{
		...
		{
			digits = 1;
			if (d != 0.0) { (void)frexp(d, &digits); }
		}
	}
	d = floor(d);
	String s(digits, csz1);
	do {
		s.Text()[--digits] = bin[uint(ldexp(modf(ldexp(d, -1), &d), 1))];
	}
	while (digits);
	return s;
}
...
static cstr homedirpath() { return getenv("HOME"); }
...
void String::ReadFromFile(cstr cpath, off_t max_sz)
{
	...
	if (!cpath || !*cpath)
	{
		errno = ENOENT;
		return;
	} // no such file or directory --> a required component of the filepath is missing
	cpath = quick_fullpath(cpath);
	if (errno) return;
...
void String::ReadFromFile(cString& path, off_t max_sz) { ReadFromFile(path.CString(), max_sz); }


/* ----	32-Bit-Primzahlen -----------------------------------------------------------------
*/
static uint32 prim[] = {
	4082442179u, 3134562593u, 2883627547u, 2567654303u, 3311094179u, 2660468347u, 3352679063u, 2179288373u, 2219291651u,
	3264466963u, 2267587997u, 3759131197u, 3484443481u, 2157055477u, 3623644897u, 3408306677u, 3534620147u, 2609476411u,
	4009698493u, 3676609817u, 4210158697u, 3710124419u, 3206152063u, 2490176341u, 2320400059u, 3324063619u, 4000354447u,
	3516401387u, 2444461157u, 3457331093u, 3272600677u, 4237679587u,
...
uint32 String::CalcHash() const
{
	...
	for (e = 0; e < Len();)
	{
		...
		case csz1:
		{
			const ucs1char* p = Ucs1() + a;
			for (i = 0; i < n; i++) hash ^= p[i] * prim[i];
		}
		break;
		case csz2:
		{
			const ucs2char* p = Ucs2() + a;
			for (i = 0; i < n; i++) hash ^= p[i] * prim[i];
		}
		break;
		case csz4:
		{
			const ucs4char* p = Ucs4() + a;
			for (i = 0; i < n; i++) hash ^= p[i] * prim[i];
		}
		break;
		}
