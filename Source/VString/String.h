...
class String
{
	...
	static size_t calc_data_and_csz(cptr p, CharSize sz)
	{
		assert(!(size_t(p) & 3));
		return size_t(p) + sz - 1;
	}
	...
	{
		assert(!(size_t(p) & 3));
		data_and_csz = (data_and_csz & 3) + size_t(p);
	}
	void SetCsz(CharSize sz) { data_and_csz = (data_and_csz & size_t(~3)) + sz - 1; }
	void SetDataAndCsz(cptr p, CharSize sz)
	{
		assert(!(size_t(p) & 3));
		data_and_csz = size_t(p) + sz - 1;
	}
...
public:
	...
	const ucs1char* UCS1Text() const
	{
		assert(Csz() == csz1);
		return Ucs1();
	}
	const ucs2char* UCS2Text() const
	{
		assert(Csz() == csz2);
		return Ucs2();
	}
	const ucs4char* UCS4Text() const
	{
		assert(Csz() == csz4);
		return Ucs4();
	}
