...
class Vek;
typedef const Vek cVek;
class Proc;
typedef const Proc cProc;
class VarPtr;
typedef const VarPtr cVarPtr;
...
enum { // protection flags+counters
...
class Var
{
	...
	// access type:
	void init_type(vtype type = isNumber)
	{
		type_and_lock = (type_and_lock & ~type_mask) + type;
	} // also preserve type and data lock
	void kill_type() {}
	void set_type(vtype type = isNumber)
	{
		kill_type();
		init_type(type);
	}
	...
	// access name:
	void init_name(NameHandle nh = 0)
	{
		name = nh;
		if (nh) LockNameHandle(nh);
	}
	void init_name(cstr s) { name = NewNameHandle(s); }
	void init_name(cString& s) { name = NewNameHandle(s); }
	void kill_name()
	{
		if (name) UnlockNameHandle(name);
	}
	void kill_name_nop()
	{ /* when you are sure that it's unnamed */
	}
	void set_name(NameHandle nh = 0)
	{
		kill_name();
		init_name(nh);
	}
	void set_name(cstr s)
	{
		kill_name();
		init_name(s);
	}
	void set_name(cString& s)
	{
		kill_name();
		init_name(s);
	}
	...
	void kill_link_nop()
	{ /* parent==0 or parent->list().array is mangled manually */
	}
	bool is_linked() { return parent != nullptr; }
	bool is_unlinked() { return parent == nullptr; }
	void link(Var* par, uint idx)
	{
		xxassert(!is_linked());
		kill_link_nop();
		init_link(par, idx);
	...
	void unlink()
	{
		xxassert(!is_unlinked());
		kill_link();
		init_link();
		unlock();
	}
	void relink(Var* par, uint idx)
	{
		xxassert(!is_unlinked());
		kill_link();
		init_link(par, idx);
	...
	const Handle& handle() const { return handle_value; }
	...
	void init_data_and_lock()
	{
		init_type_and_lock();
		init_value();
	}
	void init_data_and_lock(vtype t)
	{
		init_type_and_lock();
		init_data(t);
	}
	void init_data_and_lock(cVar& v)
	{
		init_type_and_lock();
		init_data(v);
	}

	void init_data_and_lock(Double d)
	{
		init_type_and_lock();
		init_value(d);
	}
	void init_data_and_lock(cString& s)
	{
		init_type_and_lock(isText);
		init_text(s);
	}
	void init_data_and_lock(cVek& l)
	{
		init_type_and_lock(isList);
		init_list(l);
	}
	void init_data_and_lock(cProc& p)
	{
		init_type_and_lock(isProc);
		init_proc(p);
	}
	void init_data_and_lock(Var* v)
	{
		init_type_and_lock(isVarRef);
		init_varptr(v);
	}
	void init_data_and_lock(Stream* s)
	{
		init_type_and_lock(isStream);
		init_handle(s, isStream);
	}
	void init_data_and_lock(Sema* s)
	{
		init_type_and_lock(isSema);
		init_handle(s, isSema);
	}
	void init_data_and_lock(Irpt* i)
	{
		init_type_and_lock(isIrpt);
		init_handle(i, isIrpt);
	}
	void init_data_and_lock(Thread* t)
	{
		init_type_and_lock(isThread);
		init_handle(t, isThread);
	}
	...
	void init_data(cString& s)
	{
		init_type(isText);
		init_text(s);
	}
	void init_data(Stream* s)
	{
		init_type(isStream);
		init_handle(s, isStream);
	}
	...
	void set_data(vtype v)
	{
		kill_data();
		init_data(v);
	}
	void set_data(cString& s)
	{
		kill_data();
		init_data(s);
	}
	void set_data(cVar& q)
	{
		kill_data();
		init_data(q);
	}
	void set_data(Stream* s)
	{
		kill_data();
		init_data(s);
	}
...
#define Validate() validate(__FILE__, __LINE__)
	...
	{
		xassert(!(static_cast<Var*>(p))->is_locked());
		delete_var(static_cast<Var*>(p));
	}
	...
	Var(Var* par, uint idx, const char* const* argv, uint argc = ~0u);
	Var(const char* const* argv, uint argc = ~0u);
	Var(cProc& p)
	{
		init_name();
		init_link();
		init_data_and_lock(p);
	}
	explicit Var(Thread* t)
	{
		init_name();
		init_link();
		init_data_and_lock(t);
	}
	explicit Var(Stream* s)
	{
		init_name();
		init_link();
		init_data_and_lock(s);
	}
	...
	Var& operator=(cString& s)
	{
		set_data(s);
		return *this;
	} // note: no test for intersection
	Var& operator=(Stream* s)
	{
		set_data(s);
		return *this;
	}
	...
	void Protect()
	{
		for (Var* p = this; p; p = p->parent) p->lock();
	}
	void Unprotect()
	{
		Var* p = this;
	...
	} // top parent incl. entire tree may vanish
	bool IsProtected() const
	{
		cVar* p = this;
		while (p && p->is_locked()) p = p->parent;
		return p == nullptr;
	}
	...
	{
		lock_type();
		setmask(type_lock_mask);
	}
	void DeepUnlockType()
	{
		unlock_type();
		resmask(~type_lock_mask);
	}
	...
	{
		lock_data();
		setmask(data_lock_mask);
	}
	void DeepUnlockData()
	{
		unlock_data();
		resmask(~data_lock_mask);
	}
	...
	bool operator==(cVar& q) const
	{
		return get_type() == q.get_type() && (IsNumber() ? value() == q.value() : compare(q) == 0);
	}
...
inline VarPtr::VarPtr(Var* vp)
{
	assert(vp);
	(var = vp)->Lock();
}
inline Var* VarPtr::operator=(Var* q_var)
{
	assert(q_var);
	q_var->Lock();
	var->Unlock();
...
inline Var* VarPtr::operator=(cVarPtr& q)
{
	q.var->Lock();
	var->Unlock();
	var = q.var;
...
inline void Init(Var*& ptr, Var* var)
{
	assert(var);
	var->Lock();
	ptr = var;
}
inline void Set(Var*& ptr, Var* var)
{
	assert(var);
	var->Lock();
	ptr->Unlock();
...
inline Proc::Proc(cProc& q)
{
	(bundle = q.bundle)->lock();
	ip = q.ip;
}
inline Proc::Proc(Var* v, uptr p)
{
	(bundle = v)->lock();
	ip = p;
}
inline Proc::~Proc()
{
	if (bundle->is_not_double_locked()) logline("((purge NH in proc core))");
	bundle->unlock();
}

inline void Proc::init(cProc& q)
{
	(bundle = q.bundle)->lock();
	ip = q.ip;
}
inline void Proc::init(Var* r, uptr p)
{
	(bundle = r)->lock();
	ip = p;
}
