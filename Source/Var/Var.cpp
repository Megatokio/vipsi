...
#define VTRAP(X) \
  if (!(X)) {}   \
  else abort("internal error in file %s line %u: %s", file, line, "VTRAP(" #X ")")
	...
	if (IsText()) { text().Check(file, line); }

#undef VTRAP
}

#define XXXXCHECK(v) \
  if (SAFE >= 4) (v)->validate(__FILE__, __LINE__)
#define XXXCHECK(v) \
  if (SAFE >= 3) (v)->validate(__FILE__, __LINE__)
#define XXCHECK(v) \
  if (SAFE >= 2) (v)->validate(__FILE__, __LINE__)
#define XCHECK(v) \
  if (SAFE >= 1) (v)->validate(__FILE__, __LINE__)
...
void Var::set_error_locked()
{
	...
	else { SetError(vtypeislocked, String("variable ") + GetNameForHandle(name).ToQuoted() + ": type is locked"); }
}
...
inline void Var::init_link_grow(Var* par, uint idx)
{
	...
	else { xxassert(par->list().array[idx] == nullptr); }
...
void Var::Unlink()
{
	...
	Var* par = parent;
	if (par->data_is_locked()) { par->set_error_locked(); }
	else
	{
		kill_link();
		init_link();
...
void Var::move_data(Var& q)
{
	...
	case isVarRef: varptr() = q.varptr(); break; // move pointer
	...
	case isText: text()._move(q.text()); break;

	case isNumber: value() = q.value(); break;
	}
}
...
*/
Var::Var(const char* const* argv, uint n)
{
	...
	cstr s;
	for (uint i = 0; i < n && (s = argv[i]); i++) { new Var(this, i, s); }
...
Var& Var::operator=(Var& q)
{
	...
	if (this != &q)
	{
		...
		if (type_or_data_locked() && (data_is_locked() || get_type() != q.get_type())) { set_error_locked(); }
		else
		{
...
void Var::CopyData(Var& q)
{
	...
	if (type_or_data_locked() && (data_is_locked() || q.get_type() != get_type())) { set_error_locked(); }
	else
	{
		kill_data();
		init_data(q);
...
void Var::SwapData(Var& q)
{
	...
	if (type_or_data_locked() && (data_is_locked() || q.get_type() != get_type())) { set_error_locked(); }
	else if (q.type_or_data_locked() && (q.data_is_locked() || q.get_type() != get_type())) { q.set_error_locked(); }
	else
	{
		swap_data(q);
		XXXCHECK(this);
...
Var* Var::FindItem(NameHandle namehandle) const
{
	...
	if (IsList())
	{
		...
		while (e > a)
		{
			...
			if (v)
			{
				if (namehandle != v->name) continue;
			}
			else
			{
				if (namehandle != 0) continue;
				else v = new Var(const_cast<Var*>(this), uint(e - a));
			}
			return v;
		}
	}
...
int32 Var::Find(cVar& v, int32 startidx) const
{
	...
	if (IsList())
	{
		...
		while (startidx < endidx)
		{
			...
			if (a == nullptr ? visnull : v.get_type() == a->get_type() && v.compare(*a) == 0) { return startidx - 1; }
		}
	}
...
void Var::InsertItem(uint idx, Var* q)
{
	...
	if (data_is_locked()) { set_error_locked(); }
	else if (q->parent && q->parent->data_is_locked()) { q->parent->set_error_locked(); }
	else
	{
		insert_items(idx, 1);
		if (q->parent) { q->kill_link(); }
		else
		{
			q->kill_link_nop();
			q->lock();
...
void Var::AppendItems(Var* a, Var* b, Var* c, Var* d)
{
	...
	xxassert(
		!d || !d->IsStream() ||
		(d->GetStream()->InputEncoding() >= ucs1 && d->GetStream()->InputEncoding() <= utf8 &&
		...
		void Var::sort(SortProcPtr sorter)
{
	...
	if (IsList() && list().used >= 2)
	{
		...
		else
		{
			...
			for (uint i = n; i--;)
			{
				if (qlist[i] == nullptr) qlist[i] = zero;
			}
		...
		for (uint i = 0; i < list().used; i++)
		{
			...
			if (v)
			{
				...
				if (v->IsNamed()) // include name ?
				{
					...
					for (int i = 1; f && i < v_name.Len(); i++)
					{
						c = v_name[i];
						f = ucs4::is_letter(c) || ucs4::is_dec_digit(c) || c == '_';
					}
...
void Var::JoinLines(cString& sep)
{
	...
	if (IsNoList()) { ConvertToString(); }
	else if (list().used == 0) { SetString(); }
	else
	{
		...
		for (uint i = 0; i < list().used; i++)
		{
			if (i)
			{
				UCSCopy(csz, z, sep.Csz(), sep.Text(), sep.Len());
				z += csz * sep.Len();
			}
			cString& q = list().array[i]->text();
			UCSCopy(csz, z, q.Csz(), q.Text(), q.Len());
			z += csz * q.Len();
		}
...
Var& Var::operator>>=(Double q)
{
	if (IsNumber())
	{
		if (data_is_locked()) goto e;
	a:
		value() = ldexp(value(), -int(floor(0.5 + q))); /* uint32(value())>>int(q); */
		return *this;
	}
	if (IsNoList())
	{
...
Var& Var::list_op(Var& (Var::*op)(Double), cVar& q)
{
	...
	if (IsList())
	{
		...
		if (q.Contains(this) || this->Contains(q))
			if (this != &q) return list_op(op, Var(q));
...
#define OPERATOR(OP, OPG)                       \
  ...
  Var Var::OP(Double q) const                   \
  {                                             \
	Var z(*this);                               \
	z.OPG(q);                                   \
	return z;                                   \
  }                                             \
  Var Var::OP(cVar& q) const                    \
  {                                             \
	Var z(*this);                               \
	z.OPG(q);                                   \
	return z;                                   \
...
Var& Var::append_list(cVar& q, DisassProcPtr disass_proc)
{
	...
	if (IsList()) // list #= list
	{
		...
		while (n--)
		{
			...
			if (qp->IsText())
			{
				zp->AppendString(qp->text(), disass_proc);
				continue;
			}
			if (qp->IsNoList())
			{
				zp->AppendString(qp->ToString(no, disass_proc), disass_proc);
				continue;
			}
	...
	else
	{
		...
		while (n--)
		{
			...
			if (qp->IsNoList())
			{
				zp->text() += qp->ToString(no, disass_proc);
				continue;
			}
			zp->append_list(*qp, disass_proc);
		}
...
Var& Var::AppendString(cString& q, DisassProcPtr disass_proc)
{
	if (IsText())
	{
		if (data_is_unlocked()) text() += q;
		else set_error_locked();
	}
