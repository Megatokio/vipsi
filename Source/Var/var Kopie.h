...
class Var
{
	...
	const Handle& handle() const { return *(Handle*)data; }
...
#define Validate() validate(__FILE__, __LINE__)
	...
	Var(Var* par, uint idx, const char* const* argv, uint argc = ~0);
	Var(const char* const* argv, uint argc = ~0);
	Var(cProc& p)
	{
		init_name();
		init_link();
