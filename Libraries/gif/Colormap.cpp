...
int
Colormap::addColor ( cComp q[3] ) throw(bad_alloc,limit_error)
{
	int n=used_colors++;
...
int
Colormap::condAddColor ( cComp q[3] ) throw(bad_alloc,limit_error)
{
	int i = findColor(q);
...
int
Colormap::condAddColor ( Comp r, Comp g, Comp b ) throw(bad_alloc,limit_error)
{
...
void
Colormap::shrinkCmap ( int n ) throw(limit_error)
{
	n=valid_count(n);
	Cmap::Shrink(n*3);
...
void
Colormap::shrinkColors ( int n ) throw(limit_error)
{
	int m=valid_count(n);
	Cmap::Shrink(m*3);
	if( used_colors>n ) used_colors=n;
