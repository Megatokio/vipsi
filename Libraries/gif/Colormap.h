...
#ifndef GifColormap_h
#define GifColormap_h
...
static int	valid_count		( uint16 n )	{ XXLIMIT(n,256); return n==0 ? 0 : 1<<bits(n); }
...
// create, copy, destroy:
			Colormap		()							throw()						  { init(); }
			Colormap		( int n )					throw(bad_alloc,limit_error) : Cmap(valid_count(n)*3) { init(); }
			Colormap		( cComp* q, int n, int i=unset )
														throw(bad_alloc,limit_error) : Cmap(valid_count(n)*3) { init(q,n,i); }
//			~Colormap		()							{ kill(); }
...
// color map:
	...
	Comp*	getCmap			() throw()					{ return array; }
	cComp*	getCmap			() const throw()			{ return array; }
	int		cmapByteSize	() const throw()			{ return count; }

	void	purgeCmap		() throw()					{ Cmap::Purge(); init(); }
	int		cmapSize		() const throw()			{ return count/3; }
	int		cmapBits		() const throw()			{ return bits(count/3); }
	void	growCmap		( int n ) throw(bad_alloc,limit_error)	{ n=valid_count(n); Cmap::Grow(n*3); }
	void	shrinkCmap		( int n ) throw(limit_error);


// transparent color:

	bool	hasTranspColor	() const throw()			{ return transp!=unset; }
	int		transpColor		() const throw()			{ return transp; }
	void	setTranspColor	( int i ) throw(index_error){ XXLIMIT(i+1,used_colors+1); transp = i; }
	void	clearTranspColor() throw()					{ transp = unset; }
	int		addTranspColor	() throw(limit_error)		{ if(transp==unset) { transp = addColor(0,0,0); } return transp; }


// actually used colors:

	int		usedColors		() const throw()			{ return used_colors; }
	int		usedBits		() const throw()			{ return bits(used_colors); }
	void	growColors		( int n ) throw(bad_alloc,limit_error){ if(n>used_colors) { growCmap(n); used_colors=n; } }
	void	shrinkColors	( int n ) throw(limit_error);

	Comp*	operator[]		( int i ) throw(index_error)		{ XXINDEX(i,used_colors); return array+i*3; }
	cComp*	operator[]		( int i ) const throw(index_error)	{ XXINDEX(i,used_colors); return array+i*3; }

	int		addColor		( Comp r, Comp g, Comp b )	throw(bad_alloc,limit_error) { Comp q[]={r,g,b}; return addColor(q); }
	int		addColor		( cComp q[3] )				throw(bad_alloc,limit_error);

	int		condAddColor	( cComp q[3] )				throw(bad_alloc,limit_error);
	int		condAddColor	( Comp r, Comp g, Comp b )	throw(bad_alloc,limit_error);
