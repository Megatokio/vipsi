...
class GifEncoder
{
	...
	void		write_gif_code	(int) throw(file_error);
	void		write_gif_byte  (int) throw(file_error);
...
*/
	void    openFile			(cstr path)     throw(file_error);
...
*/
	void writeScreenDescriptor ( uint16 w, uint16 h, uint16 colors, uint8 aspect=0 ) throw(file_error)
	{
		XXASSERT( colors>0 );
		global_cmap.purgeCmap();
		uint8 flags = (reqBits(colors)-1)<<4;
...
*/
	void writeScreenDescriptor ( uint16 w, uint16 h, Colormap const& cmap, uint8 aspect=0 ) throw(file_error)
	{
		XXASSERT( cmap.usedColors()>0 );
		global_cmap = cmap;
...
*/
	void closeFile			() throw(file_error);


/*	Compression of image pixel stream:
*/
	void startImage			( int bits_per_pixel ) throw(file_error);
	void writePixelRow		( cuptr pixel, uint w ) throw(file_error);
	void writePixelRect		( cuptr pixel, uint w, uint h, uint dy=0 ) throw(file_error);
	void finishImage		() throw(file_error);
	void writePixelmap		( int bits_per_pixel, Pixelmap const& pixel ) throw(file_error);


/*	Write different kinds of blocks:
*/
	void writeGif87aHeader	() throw(file_error)		{ fd.write_bytes( "GIF87a", 6 ); }
	void writeGif89aHeader	() throw(file_error)		{ fd.write_bytes( "GIF89a", 6 ); }
	void writeGifTrailer	() throw(file_error)		{ fd.write_uchar( 0x3B ); }


/*	Write descriptor for following sub image:
*/
	void writeImageDescriptor ( uint16 x, uint16 y, uint16 w, uint16 h, uint8 flags=0 ) throw(file_error)
	{
		XXLogLine("writeImageDescriptor: x=%i, y=%i, w=%i, h=%i",x,y,w,h);
...
*/
	void writeColormap ( Colormap const& cmap ) throw(file_error)	{ fd.write_bytes( cmap.getCmap(), cmap.cmapByteSize() ); }
	void writeColormap ( Colormap const* cmap ) throw(file_error)	{ writeColormap(*cmap); }


/*	Enable looping animations.								GIF89a++
*/
	void writeLoopingAnimationExtension ( uint16 max_loops = 0 ) throw(file_error)
	{
...
*/
	void writeCommentBlock ( cstr comment ) throw(file_error)
	{
		int n = (int)strlen(comment); XXXLIMIT(n,255);
...
*/
	void writeGraphicControlBlock ( uint16 delay/*1/100sec*/, int transp_index = Colormap::unset ) throw(file_error)
	{
...
*/
	void writeImage ( Pixelmap* pm, cColormap* cm ) throw(file_error);
	void writeImage ( Pixelmap& pm, cColormap& cm ) throw(file_error)	{ writeImage(&pm,&cm); }
	void writeImage ( Pixelmap& pm )               throw(file_error)    { writeImage(&pm,&global_cmap); }
};
