...
/*	Copy Creator:
*/
void Pixelmap::init( cPixelmap& q ) throw(bad_alloc)
{
...
{
//	LogLine("Pixelmap::reduceColors():  TESTWEISE DISABLED!");
//	return;

	XLogIn  ( "Pixelmap::reduceColors(Colormap)" );
//	XXLogLine( "old colors: %i", (int)cmap.usedColors() );
//	if( XXLOG && cmap.hasTranspColor() ) LogLine( "old transp: %i",(int)cmap.transpColor() );
//	XXLogLine( "cmap: %s",cmap.cmapStr() );
	...
	int  w  = width();
	int  h  = height();
	int  dy = rowOffset(); XXXASSERT(dy>=w);
	...
	for( int x=0; x<w; x++ )
	{
		...
		if( k>=n )
		{
			XXASSERT(c<cmap.usedColors());
			my_conv[c] = k = n++;
			my_cmap.addColor(cmap[c]);

//			XXLogLine( "  pixel[x=%i,y=%i] = %i -> %i, rgb=(%hhu,%hhu,%hhu)",
//				x1()+x, y1()+y, c, k, cmap[c][0], cmap[c][1], cmap[c][2] );
		}
		p[x] = k;
	}

	XXXASSERT( n <= cmap.usedColors() );
	XXXASSERT( n == my_cmap.usedColors() );
	...
	if( cmap.hasTranspColor() )
	{
		...
		if( my_transp<n )
		{
			...
			XXLogLine( "new transp: %i",(int)my_cmap.transpColor() );
		}
		else
		{
			XXLogLine( "no transparent pixels!" );
		}
	}

	cmap = my_cmap;
//	XXLogLine( "new colors: %i", (int)cmap.usedColors() );
//	XXLogLine( "cmap: %s",cmap.cmapStr() );
}
...
/*	Set this Pixmap to the difference Pixmap which converts this Pixmap into the new Pixmap.
