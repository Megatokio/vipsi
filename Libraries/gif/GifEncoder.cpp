...
extern cstr	appl_version_str;		// "0.6"
extern cstr	appl_pre_version_str;	// "pre2" or ""
...
*/
void GifEncoder::write_gif_byte( int ch ) throw(file_error)
{
	if( file_state == IMAGE_COMPLETE ) return;

	XXASSERT(bufsize<=255);
...
*/
void GifEncoder::write_gif_code( int code ) throw(file_error)
{
...
/*	Initialise the lzw encoder
	in: cmap depth (bits).
*/
void GifEncoder::startImage( int cmap_bits ) throw(file_error)
{
	XLogIn("GifEncoder:StartImage(bits)");

	XXASSERT( fd.is_valid() );				// file must already be opened with CreateFile()
...
*/
void GifEncoder::writePixelRow( cuptr pixel, uint length ) throw(file_error)
{
	XXXLogIn("GifEncoder:WritePixelRow(px,n)");
...
*/
void GifEncoder::writePixelRect ( cuptr pixel, uint w, uint h, uint dy ) throw(file_error)
{
	XXLogIn("GifEncoder:WritePixelRect(px,w,h,dy)");

	XXASSERT( (int16)dy>=(int16)w );
	XXASSERT( w<0x8000 && h<0x8000 );
...
*/
void GifEncoder::finishImage( ) throw(file_error)
{
...
*/
void GifEncoder::writePixelmap ( int bits_per_pixel, cPixelmap& pm ) throw(file_error)
{
	XXLogIn("GifEncoder:WritePixelmap(bits,pmap)");
...
*/
void GifEncoder::openFile( cstr path ) throw(file_error)
{
	XLogIn("GifEncoder:openFile()");
	XXASSERT( !fd.is_valid() );
...
/*	Write Comment Block with creator message
	...
	Close file
*/
void GifEncoder::closeFile() throw(file_error)
{
	XLogIn("GifEncoder:CloseFile()");
	XXASSERT(fd.is_valid());

	if(file_state!=IMAGE_COMPLETE) finishImage();
//	writeCommentBlock( usingstr("Made with %s%s%s",appl_name,appl_version_str,appl_pre_version_str) );
	writeGifTrailer();
	fd.close_file();
}
...
*/
void GifEncoder::writeImage (Pixelmap* pmap, cColormap *cmap ) throw(file_error)
{
	XXLogIn("GifEncoder:WriteImage(pmap,cmap)");
