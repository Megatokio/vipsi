/*	Copyright  (c)	Günter Woigk 2004 - 2015
...
class PLock
{
	pthread_mutex_t mutex;
            ...
            PLock       (PLock const&);     // prohibit
    PLock&  operator=   (PLock const&);     // prohibit
...
public:
	enum Attr   // for usage with PLock(Attr)
	{
		...
		errorcheck	= PTHREAD_MUTEX_ERRORCHECK		// w/ usage error checking
			...
			PLock	()			{ int e = pthread_mutex_init(&mutex,NULL); 	XASSERT(!e); }
			PLock	(Attr);
			~PLock	()          { int e = pthread_mutex_destroy(&mutex); 	XXXASSERT(!e); }

	void	lock	() volatile throw()	{ int e = pthread_mutex_lock((pthread_mutex_t*)&mutex);   	XXXASSERT(!e); }
	void	unlock	() volatile throw()	{ int e = pthread_mutex_unlock((pthread_mutex_t*)&mutex); 	XXXASSERT(!e); }
	bool	trylock	() volatile throw()	{ return pthread_mutex_trylock((pthread_mutex_t*)&mutex)==0; }	// 1 = success
};
...
inline void  assertMainThread()	{ XXXASSERT(isMainThread()); }
